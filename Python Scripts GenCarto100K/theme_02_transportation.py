...
def thin_road_network_(in_features, minimum_length_min, minimum_length_max, invisibility_field, hierarchy_field, ref_scale, carto_high_sql, carto_low_sql, carto_partition, working_gdb):
    arcpy.AddMessage("Starting thin road networking")
    try:
        # Set the workspace
        arcpy.env.overwriteOutput = True
        ...
        fc_singlepart_road_lyr=arcpy.management.MakeFeatureLayer(fc_singlepart_road, "fc_singlepart_road_lyr")
        fc_singlepart_track_lyr=arcpy.management.MakeFeatureLayer(fc_singlepart_track, "fc_singlepart_track_lyr")

        arcpy.AddMessage(f"{minimum_length_max} Meters")
       
        arcpy.cartography.ThinRoadNetwork(
                f"{fc_singlepart_road_lyr};{fc_singlepart_track_lyr}",
                f"{minimum_length_max} Meters",
                invisibility_field,
                hierarchy_field
            )
