...
def vegetation_symbol_create(fc_list, map_name_symbology,  layer_details, map_scale, road_track_buffer_distance, map_unit, logger):
    ...
    try: 
        ...
        road_data = arcpy.analysis.Buffer(road_line, "road_line_buffer", f"{road_track_buffer_distance} Meters")
        track_data = arcpy.analysis.Buffer(track_line, "track_line_buffer", f"{road_track_buffer_distance} Meters")
        ...
        poly_layer = [layer for layer in layer_list if layer.name == layer_details["name"]]
        
        # point_layer = [layer for layer in layer_list if layer.name == point_name][0]
        # arcpy.AddMessage(f"point_layer: {point_layer}")
        if len(poly_layer) > 0:
            poly_layer = poly_layer[0]
            arcpy.AddMessage(f"poly_layer: {poly_layer}")
            arcpy.AddMessage(f"started ConvertMarkerPlacementToPoints")
            ...
            selected_moved_point = arcpy.management.SelectLayerByLocation(moved_point,  "INTERSECT", dissolved_merged_data)
...
def apply_carto_symbology(fc_list, attribution_fc_list, express_list, query_list, field_list, intersecting_fc_list, working_gdb, feature_loc, vst_workspace, hierarchy_file, prep_line_resolve_fcs_list, 
    ...
    try:
        ...
        for vegetation_fc in create_vegetation_symbol_detail:
            vegetation_symbol_create(fc_list, map_name, vegetation_fc, val_dict['ApplyCarto_Map_Scale'], val_dict['ApplyCarto_Vegetation_RoadTrackDistance'], val_dict['ApplyCarto_Map_Unit_of_Dataset'], logger)
